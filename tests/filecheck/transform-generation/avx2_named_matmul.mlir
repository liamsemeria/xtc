...
// CHECK-NEXT:      linalg.matmul {__my_matmul__, loop.interchange = ["i", "j", "k", "k1", "j1"], loop.tiles_names = {i = [], j = ["j1"], k = ["k1"]}, loop.tiles_sizes = {j1 = 64 : i64, k1 = 8 : i64}, loop.unroll = {k1 = 8 : i64}, loop.vectorize = ["j1"]} ins(%arg0, %arg1 : memref<256x512xf32>, memref<512x256xf32>) outs(%arg2 : memref<256x256xf32>)
// CHECK-NEXT:      return
// CHECK-NEXT:    }
...
// CHECK-NEXT:      %1 = transform.structured.match attributes {__my_matmul__} in %arg0 : (!transform.any_op) -> !transform.any_op
...
// CHECK-NEXT:      transform.annotate %loops_3 "__my_matmul__i" : !transform.any_op
...
// CHECK-NEXT:      transform.annotate %loops_5 "__my_matmul__j" : !transform.any_op
...
// CHECK-NEXT:      transform.annotate %loops_7 "__my_matmul__k" : !transform.any_op
...
// CHECK-NEXT:      transform.annotate %loops_9 "__my_matmul__k1" : !transform.any_op
...
// CHECK-NEXT:      %4 = transform.structured.match attributes {__my_matmul__k1} in %3 : (!transform.any_op) -> !transform.any_op
