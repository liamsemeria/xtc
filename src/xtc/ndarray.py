...
def test_ndarray():
    ...
    print(
        f"Init NPArray: dims = {len(npin.shape)}, shape = {npin.shape}, dtype = {npin.dtype}, data = {npin.data}"
    )
    array = NDArray(npin)
    print(
        f"To NDArray: dims = {array.dims}, shape = {array.shape}, dtype = {array.dtype}, size = {array.size}, data = {array.data}"
    )
    npout = array.numpy()
    print(
        f"To NPArray: dims = {len(npout.shape)}, shape = {npout.shape}, dtype = {npout.dtype}, data = {npout.data}"
    )
